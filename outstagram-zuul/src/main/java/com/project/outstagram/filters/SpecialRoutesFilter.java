...
import com.project.outstagram.model.AbTestingRoute;
...
public class SpecialRoutesFilter extends ZuulFilter {
    ...
    public boolean useSpecialRoute(AbTestingRoute testRoute){
        ...
        //경로가 활성 상태(active) 인지 확인한다.
        if (testRoute.getActive().equals("N")) return false;

        //대체 서비스 경로의 사용 여부를 결정한다.
    ...
    public Object run() {
        ...
        // 이 서비스에 대한 라우팅 레코드가 있는지 조회
        AbTestingRoute abTestRoute = getAbRoutingInfo( filterUtils.getServiceId() );
        // 난수를 생성하고 요청을 대체 서비스로 전달할지 결정
        if (abTestRoute!=null && useSpecialRoute(abTestRoute)) {
            //서비스가 지정한 서비스 위치의 완전한 URL을 만든다.
