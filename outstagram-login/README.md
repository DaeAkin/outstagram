...
마이크로서비스 애플리케이션은 수많은 작은 서비스로 분산되어 상호 작용하며 서로가 관리하는 데이터에 관심이 있다. HTTP 설계를 가진 동기식 설계에서 본 것처럼 동기식 HTTP 응답은 다른 마이크로서비스 간의 강한 의존성을 만든다. 이 의존성을 완전히 제거할 수는 없지만 서비스가 소유한 데이털르 직접 관리하는 엔드포인트만 노출함으로써 의존성을 최소화할 수 있다. 메세징 방식에서는 상태 ㅂㅕㄴ화를 전달하는 과정에서 두 서비스가 서로 알지 못하므로 결합되지 않는다. 조직 서비스는 상태 변화를 발행해야 할 때 큐에 메세지를 기록하며, 라이선싱 서비스는 메세지 수신 여부만 알고 누가 메세지를 발행했는지 알 수 없다.

### 내구성 

큐가 존재하므로 서비스 소비자가 다운될 때도 메세지 전달을 보장할 수 있으며, 조직 서비스는 라이선싱 서비스가 가용하지 않더라도 메세지를 계속 발행할 수 있다. 메세지는 큐에 저장되고 라이선싱 서비스가 다시 가용될 때까지 유지될 것이다. 반대로 캐시와 큐 방식을 조합하면 조직 서비스가 다운될 때도 조직 데이터의 일부분이 캐시에 유지되므로 라이선싱 서비스가 적절히 기능을 저하시킬 수 있다. 때때로 지난 데이터라도 있는 것이 아에 없는 것보다 낫다.  

### 확장성

메세지가 큐에 저장되므로 메세지 발신자는 메세지 소비자의 응답을 기다릴 필요가 없다. 발신자는 하던 일을 계속하면 된다. 마찬가지로 큐에서 메세지를 읽어 오는 소비자가 메세지를 빠르게 처리하지 못한다면, 메세지 소비자를 더 많이 가동시켜 큐의 메세지를 신속히 처리한느 것은 큰 일이 아니다. 새 마이크로서비스 인스턴스를 손쉽게 가동시키고 추가된 서비스가 큐에 보관 중인 메세지를 쉽게 처리해야 하므로 이러한 확장적 접근 방법은 마이크로서비스 모델과 잘 들어맞는다. 이 확장성이 수평 확장의 한 예다. 큐에서 메세지를 읽어 오는 것과 관련한 전통적인 확장 메커니즘은 메세지 소비자가 한 번에 처리할 수 있는 스레드 개수를 늘리는 것인데, 안타깝게도 이 방식은 결국 메세지 소비자가 사용할 수 있는 CPU 개수에 제한을 받는다. 마이크로서비스 모델은 멧지 소비자의 호스팅 머신을 늘려 확장하기 때문에 이러한 제약이 없다.

### 유연성

메세지 발신자는 누가 메세지를 소비할지 모른다. 즉 원래 발신 서비스에 영향을 주지 않고 새로운 메세지 소비자(및 새로운기능)를 쉽게 추가할 수 있다. 이것은 기존 서비스를 건드리지 않고 새로운 기능을 애 플리케이션에 추가할 수 있기 때문에 매우 강력한 개념이다. 새로운 기능의 코드는 발행되는 이벤트를 수신해서 적절히 대응할 수 있다.

### 메세지 아키텍처의 단점

다른 아키텍처 모델처럼 메세징 기반의 아키텍처는 장단점이 있다. 메세징 기반의 아키텍처는 복잡해질 수 있어 개발 팀은 다음 사항을 포함한 몇 가지 주요 사항에 주의를 기울어야 한다.

- 메세지 처리의 의미론(message handling semantics)
- 메시지 가시성(message visibility)
- 메세지 코레오그래피(message choreogrphy)

### 메세지 처리의 의미론

마이크로서비스 기반 애플리케이션에서 메세지를 사용하려면 메세지를 발행하고 소비하는 방법을 아는 것 이상이 필요하다. 애플리케이션이 <u>메세지의 소비 순서를 기반</u>으로 어떻게 동작할지와 <u>메세지가 순서대도 처리</u>되지 않을 때 어떤 일이 발생할지 이해해야 한다. 예를 들어 한 고객의 모든 주문을 받은 순서대로 처리해야 한다는 엄격한 요구 사항이 있다면, 모든 메세지를 서로 독립적으로 소비하는 방식과 다르게 처리하도록 설정하고 구성해야 할 것이다.

이것은 메세징을 사용해 데이터의 상태 전이를 엄격하게 다루어야 할 때 예외가 발생하거나 순서대로 처리되지 않는 시나리오를 애플리케이션의 설계 단계에서부터 고려해야 한다는 것을 의미한다. "메세지가 실패하면 에러 처리를 재시도할 것인가? 아니면 그냥 실패하게 놔둘 것인가? 특정 고객 메세지 중 하나가 실패할 경우 해당 고객과 관련된 향후 메세지를 어떻게 처리할 것인가?" 등은 깊이 생각할 주제다.

### 메세지 가시성

마이크로서비스에서 메세지 사용은 종종 동기식 서비스 호출과 서비스 내 비동기 처리가 합쳐진 것을 의미한다. 메세지의 비동기적 특성으로 메세지가 발행되거나 소비될 때 메세지의 수신과 처리가 아주 근접한 곳에서 이루어질 필요는 없다. 웹 서비스의 호출과 메세징을 경유하는 사용자가 트랜잭션을 추적하기 위해 상관관계 ID를 사용하는 것도 애플리케이션에서 발생하는 일을 이애하고 디버깅하는 데 매우 중요하다.

### 메세지 코레오그래피

메세지 가시성에서 짐작했듯이 메세징 기반 애플리케이션은 코드가 더 이상 단순한 요청-응답 모델을 사용해 선형적으로 처리되지 않기 애플리케이션 비 즈니스 로직을 추론하는 것은 더 어려워진다. 메세징 기반 애플리케이션의 디버깅은 사용자 트랜잭션이 순서가 바뀌고 다른 시점에 실행될 수 있는 다양한 서비스의 로그를 꼼꼼히 살펴보는 일이다.

```groovy
    // https://mvnrepository.com/artifact/org.springframework.cloud/spring-cloud-stream
    compile group: 'org.springframework.cloud', name: 'spring-cloud-stream' ,  version: '2.1.0.RELEASE'

    // https://mvnrepository.com/artifact/org.springframework.cloud/spring-cloud-starter-stream-kafka
    compile group: 'org.springframework.cloud', name: 'spring-cloud-starter-stream-kafka', version: '2.1.2.RELEASE'
```
