...
> 기본적으로 히스트릭스 팀은 대부분의 명령에 대해 기본 격리 전략인 스레드 방식을 권장한다. 스레드 격리 방식은 히스트릭스 명령 스레드와 부모 스레드 사이에 격리 수준을 높이며, 세마포어 격리 방식보다 무겁ㄷ. 세마포어 격리 모델은 경량이며, 서비스에서 대용량을 처리하고 비동기 I/O 프로그래밍 모델(예를 들어 Netty 같은 비동기 I/O 컨테이너 사용)을 적용할 때 사용해야 한다.

### HystrixConcurrencyStrategey 동작

목적? 로그 찍어주는 것 때문에 이용하는 거 같은데..
...
하.. 히스트릭스 포기. fallback 처리때문에  HystrixConcurrencyStrategey 를 사용하는 거 같은데 다시 봐바야 겠다.